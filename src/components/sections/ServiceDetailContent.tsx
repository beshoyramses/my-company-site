...
export function ServiceDetailContent({ service }: { service: Service }) {
  ...
  return (
    <>
      ...
      <article className="section-shell pb-24 pt-32 sm:pt-36">
        <div className="overflow-hidden rounded-[2.25rem] border border-white/10 bg-[#06040c]/90 shadow-[0_28px_90px_rgba(0,0,0,0.36)]">
          <div className="p-6 sm:p-8 lg:p-12">
            <div className="mb-10">
              <ServiceGallery images={service.images} title={title} />
            </div>
            ...
            <div className="flex flex-col justify-between">
              <div>
                ...
                <h1 className="mb-4 font-display text-4xl font-semibold text-white sm:text-5xl">
                  {isArabic ? `${title} في الجونة` : `${title} in El Gouna`}
                </h1>
                {discount && (
                  <div className="mb-6 inline-flex rounded-full border border-violet-400/30 bg-violet-500/12 px-4 py-2 text-sm font-medium text-violet-100">
                    ✦ {discount}
                  </div>
                )}
